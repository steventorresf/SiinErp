...
namespace SiinErp.Areas.Ventas.Business
{
    public class ClientesBusiness
    {
        public List<Clientes> GetClientes(int IdEmp)
        {
            try
            {
                ...
                List<Clientes> Lista = (from cli in context.Clientes.Where(x => x.IdEmpresa == IdEmp)
                                        join tip in context.TablasEmpresaDetalles on cli.IdDetTipoCliente equals tip.IdDetalle
        ...
        public List<Clientes> GetClientesAct(int IdEmp)
        {
            try
            {
                SiinErpContext context = new SiinErpContext();
                List<Clientes> Lista = (from cli in context.Clientes.Where(x => x.IdEmpresa == IdEmp && x.Estado.Equals(Constantes.EstadoActivo))
                                        join tip in context.TablasEmpresaDetalles on cli.IdDetTipoCliente equals tip.IdDetalle
                                        join ciu in context.Ciudades on cli.IdCiudad equals ciu.IdCiudad
                                        join dep in context.Departamentos on ciu.IdDepartamento equals dep.IdDepartamento
                                        join zon in context.TablasEmpresaDetalles on cli.IdDetZona equals zon.IdDetalle
